// Phase 4, part 2: colophon combined with BOTH palimpsest and equator
...
// add()/del() -- which, unlike equator's check(), always wraps its
...
#show: contexture.bundle.with(
  template: colophon.instrument(
    template: simple-template.with(title: [Combined test -- palimpsest + equator + colophon]),
  ),
  documents: (
